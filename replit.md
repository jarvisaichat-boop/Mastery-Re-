# Mastery Dashboard

> **DO NOT DELETE OR RENAME THIS FILE**
> This `replit.md` file is automatically read by Replit Agent to understand project context, preferences, and architecture. Deleting or renaming it will break Agent's ability to work effectively on this project.
> See also: `MASTER_CONTEXT.md` for detailed project vision and roadmap.

---

## Overview

Mastery Dashboard is a gamified, AI-powered habit coaching application designed to guide users through structured behavior change. The app combines habit tracking with psychological coaching principles, featuring a multi-phase onboarding system, gamification mechanics (streaks, celebrations, micro-wins), and a "Momentum Generator" feature to help users overcome inertia and build consistent habits.

The application uses a dark "Focus Dojo" theme and emphasizes user psychology through archetypes, personalized AI coach personas, and structured coaching cycles.

## User Preferences

Preferred communication style: Simple, everyday language.

Additional preferences:
- Enable AI to make changes to the codebase
- Provide detailed explanations for changes
- Use iterative development approach
- Ask before making major architectural changes

## System Architecture

### Frontend Architecture
- **Framework**: React 18 with TypeScript, using functional components and hooks
- **Build Tool**: Vite 5 configured to run on port 5000
- **Styling**: Tailwind CSS with custom animations and a dark theme palette
- **Icons**: Lucide React for consistent iconography
- **Gestures**: react-swipeable for touch interactions in the Momentum Generator

### State Management Pattern
- Local React state (useState/useEffect) for component-level state
- No global state library - data flows through props
- Multiple localStorage keys prefixed with `mastery-dashboard-*` for persistent data:
  - Habits, user profiles, onboarding progress, reflections, streaks, daily reasons, content library

### Application Structure
- **Entry Point**: `src/App.tsx` - main orchestrator managing views, modals, and core state
- **Components**: Located in `src/components/` - feature-specific UI components
- **Onboarding Flow**: Multi-phase system in `src/components/mastery-onboarding/` (Phase 0-7)
- **Utilities**: `src/utils/` for date handling, logging, video recommendation, goal analysis
- **Services**: `src/services/NotificationService.ts` for browser notifications with escalation logic
- **Data**: `src/data/` contains program library and content library definitions

### Key Features Architecture
1. **Momentum Generator** (`MomentumGeneratorModal.tsx`): 8-step flow to build user momentum with a 3D flip card pledge system
2. **Discipline Engine**: Tiered logging windows based on habit strictness (Anchor, Life Goal, Regular)
3. **Micro-Win Protocol**: Fallback system for overwhelming days with 60-second action options
4. **Notification Escalation**: T-5 gentle → T-0 urgent → T+5 buzzing notification pattern
5. **Walkthrough** (`src/components/VisionBoard/`): Four-section carousel (Core Values, Path, Schedule, Custom) with:
   - ActionMenu for item management (move up/down, hide/show, delete)
   - View/Edit mode toggle per section
   - Hidden items display with opacity and strikethrough in edit mode
   - Data migration for backward compatibility with legacy formats
   - **Visual Timeline Editor** in Schedule section (`VisualTimelineEditor.tsx`):
     - Google Calendar-style 24-hour vertical grid with proportionally-sized blocks
     - Drag-to-resize: drag top/bottom edges to change start/end time (15-minute snap)
     - Drag-to-move: drag block body to reposition within day
     - Tap-to-edit: click block to edit name, color picker for block colors
     - Protected blocks: Sleep and Work/School (isProtected), GM/GD/GN Routines (isRoutine) - cannot be deleted
     - Auto-generated points: Wake Up (from Sleep end time) and Sleep (from Sleep start time) markers
     - Overnight block support: Sleep blocks spanning midnight (e.g., 23:00→06:00) handled correctly
     - TimeBlock flags: isProtected (undeletable core blocks), isRoutine (routine blocks), isAutoGenerated (derived points), routineType (canonical GM/GD/GN identifier)

---

## Recent Updates (December 2024)

### Visual Timeline Editor - Complete Feature Walkthrough

The Schedule section of the Walkthrough now features a fully interactive visual timeline editor, similar to Google Calendar's day view.

#### Core Functionality
1. **24-Hour Vertical Grid**: Displays hours from 00:00 to 23:00 with a proportional layout where each hour takes equal vertical space
2. **Proportional Block Sizing**: Time blocks are sized based on their actual duration (e.g., a 2-hour block is twice as tall as a 1-hour block)
3. **Inline Hex Colors**: Block backgrounds use inline hex color styles with opacity for visibility

#### Drag Interactions
1. **Drag-to-Resize**: Grab the top or bottom edge of any block to change its start or end time
   - Snaps to 15-minute increments for precision
   - Visual feedback shows resize handles on hover/touch
2. **Drag-to-Move**: Grab the body of a block to reposition it within the day
   - Maintains block duration while changing start time
3. **Tap-to-Edit**: Click/tap any block to open an edit modal for:
   - Changing the block label/name
   - Selecting a new color from the color picker
   - Deleting non-protected blocks

#### Protected Block System
- **isProtected flag**: Sleep and Work/School blocks cannot be deleted
- **isRoutine flag**: GM, GD, and GN Routine blocks cannot be deleted
- **isAutoGenerated flag**: Wake Up and Sleep time points are derived from Sleep block times
- **routineType field**: Canonical identifier (gm/gd/gn) for routine blocks, enables robust deduplication

#### Compact Layout
- Blocks under 45 minutes use a horizontal flex layout with smaller text
- Ensures labels remain visible even on short-duration blocks

#### Overnight Block Support
- Sleep blocks can span midnight (e.g., 23:00 → 06:00)
- System correctly handles the wrap-around calculation for display and editing

### Migration & Data Cleanup

#### Duplicate Routine Cleanup
- Previous migration bug caused duplicate GM/GD/GN routine blocks to be created
- Cleanup logic now runs on every app load:
  1. Infers canonical `routineType` (gm/gd/gn) from labels, legacy keys, or time proximity
  2. Groups routine blocks by their routineType
  3. Keeps only the first (or best) block per type, removing duplicates
  4. Preserves user customizations (renamed labels, edited times)

#### Migration Strategy
- Data migrations run in `VisionBoardContext.tsx` during initial load
- Migrations are idempotent - safe to run multiple times
- Legacy format detection ensures backward compatibility with older data structures

### Backend API Server
- Express.js server running on port 3001
- Vite proxy configuration routes `/api/*` requests to backend during development
- Currently handles YouTube Data API v3 integration for video metadata and search

## External Dependencies

### Third-Party Services
- **YouTube Data API v3**: Video metadata fetching and search for content library
- **youtube-transcript**: NPM package for fetching video transcripts (server-side)

### Browser APIs
- **Notification API**: Browser push notifications with permission management
- **Speech Synthesis API**: Voice guidance in Breath Pacer component
- **LocalStorage**: All persistent data storage (no database)

### Key NPM Dependencies
- `react`, `react-dom`: UI framework
- `express`, `cors`: Backend API server
- `lucide-react`: Icon library
- `react-swipeable`: Touch gesture handling
- `youtube-transcript`: Transcript fetching

### Development Dependencies
- `vite`, `@vitejs/plugin-react`: Build tooling
- `typescript`, `typescript-eslint`: Type safety
- `tailwindcss`, `autoprefixer`, `postcss`: CSS processing
- `eslint`, `eslint-plugin-react-hooks`: Code quality