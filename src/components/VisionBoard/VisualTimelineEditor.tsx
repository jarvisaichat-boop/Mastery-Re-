...
export const VisualTimelineEditor: React.FC<VisualTimelineEditorProps> = ({ timeline, onUpdate }) => {
  ...
  const userPoints = timeline.filter(b => !b.endTime && !b.hidden);
  ...
  const handleMouseMove = useCallback((e: MouseEvent | TouchEvent) => {
    ...
    const isOvernightBlock = originalDuration > 0 && dragState.endMinutes > 24 * 60;
    ...
    if (dragState.edge === 'top') {
      ...
      if (isOvernightBlock) {
        newEnd = Math.max(newStart + 15, dragState.endMinutes + deltaMinutes);
  ...
  return (
    <div className="relative select-none">
      <div 
        ref={containerRef}
        className="relative bg-black/20 rounded-lg overflow-hidden z-0"
      ...
      >
        ...
        <div className="absolute left-10 right-2 top-0 bottom-0">
          {blocks.flatMap((block, idx) => {
            ...
            if (endMins > 24 * 60) {
              // Wrapping block
              // Part 1: Start to 24:00 (has Top handle only)
              parts.push({
                key: `${idx}-evening`,
                topPixels: minutesToPixels(startMins),
                heightPixels: minutesToPixels(24 * 60 - startMins),
                hasTopHandle: true,
                hasBottomHandle: false,
                isPart2: false
              });
              // Part 2: 00:00 to End (has Bottom handle only)
              parts.push({
                key: `${idx}-morning`,
                topPixels: 0,
                heightPixels: minutesToPixels(endMins - 24 * 60),
                hasTopHandle: false,
              ...
              parts.push({
                key: `${idx}-normal`,
                topPixels: minutesToPixels(startMins),
                heightPixels: Math.max(30, minutesToPixels(endMins - startMins)),
                hasTopHandle: true,
                hasBottomHandle: true,
                isPart2: false
              });
            ...
            // Shorter blocks get higher z-index to be clickable when overlapping
            // Base Z=10, add bonus for shortness. Dragged or Edited gets 1000.
            const zIndexScore = 10 + Math.floor((24 * 60 - totalDuration) / 10);

            return parts.map(part => {
              const isCompact = part.heightPixels < 40; // Approx < 50 mins

              return (
                <div
                  key={part.key}
                  className={`absolute left-0 right-0 rounded-lg cursor-move transition-shadow ${dragState?.blockIndex === idx ? 'shadow-lg ring-2 ring-yellow-500/50' : ''}`}
                  style={{ 
                    top: `${part.topPixels}px`,
                    height: `${part.heightPixels}px`,
                    zIndex: (dragState?.blockIndex === idx || isEditing) ? 1000 : zIndexScore
                  }}
                >
                  {part.hasTopHandle && (
                    <div
                  ...
                  <div
                    className="h-full flex border-l-4 rounded-lg overflow-hidden"
                    style={{
                  ...
                  >
                    <div className={`flex-1 ${isCompact ? 'px-2 py-1' : 'p-2'} flex ${isCompact ? 'flex-row items-center gap-2' : 'flex-col justify-center'} min-w-0`}>
                      {isEditing && !part.isPart2 ? (
                        <div className="flex flex-col gap-1 min-w-[120px] bg-gray-900/90 p-2 rounded border border-white/20 shadow-xl" onClick={e => e.stopPropagation()}>
                          <input
                            type="text"
                            value={block.label}
                            onChange={(e) => updateBlockLabel(idx, e.target.value)}
                            className="bg-transparent text-white text-sm font-bold border-b border-white/30 outline-none w-full"
                            placeholder="Label"
                            autoFocus
                          />
                          <div className="flex items-center gap-1 text-xs">
                            <input
                              ...
                              onChange={(e) => {
                                const newTimeline = [...timeline];
                                newTimeline[actualIndex] = { ...newTimeline[actualIndex], time: e.target.value };
                                onUpdate(newTimeline);
                              }}
                              className="bg-black/50 text-white rounded px-1 py-0.5 w-full outline-none border border-white/10"
                            />
                            <span>-</span>
                            <input
                              type="time"
                              ...
                              onChange={(e) => {
                                const newTimeline = [...timeline];
                                newTimeline[actualIndex] = { ...newTimeline[actualIndex], endTime: e.target.value };
                                onUpdate(newTimeline);
                              }}
                              className="bg-black/50 text-white rounded px-1 py-0.5 w-full outline-none border border-white/10"
                            />
                        ...
                        <div className={`text-white ${isCompact ? 'text-xs' : 'text-sm'} font-medium truncate`}>
                    ...
                    {colorPickerBlock === idx && !part.isPart2 && (
                      <div
