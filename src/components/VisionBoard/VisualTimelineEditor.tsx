...
export const VisualTimelineEditor: React.FC<VisualTimelineEditorProps> = ({ timeline, onUpdate }) => {
  ...
  const sleepBlock = blocks.find(b => b.label.toLowerCase() === 'sleep');
  const autoGeneratedPoints: TimeBlock[] = [];
  
  if (sleepBlock) {
    // Check if Wake Up already exists at the end time
    const wakeUpExists = userPoints.some(b =>
  ...
  const points = [...userPoints, ...autoGeneratedPoints];
