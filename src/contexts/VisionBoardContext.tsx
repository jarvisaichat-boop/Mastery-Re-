      ...
      if (stored) {
        ...
        if (!Array.isArray(timeline)) {
          ...
          timeline = timeline.map((block: Partial<TimeBlock> & { type?: string; routineKey?: string }) => {
            ...
            // become point markers. Exact-match the known legacy labels so we
            // do not upgrade a user-created block that happens to contain the
            // word "momentum".
          ...
          // Wake Up is no longer auto-generated at render time; the Sleep
          // block's end time is the single source of truth for when the day
          // starts. Wake Up blocks with endTime are user-created and should
          // be preserved.
          timeline = timeline.filter(b => {
