      ...
      if (stored) {
        ...
        if (!Array.isArray(timeline)) {
          ...
          timeline = timeline.map((block: Partial<TimeBlock> & { type?: string; routineKey?: string }) => {
            ...
            // Migration: legacy "App Open" / "Momentum Generator" momentum blocks
            // become point markers (Task #86). Use exact matches to avoid
            // accidentally upgrading a user-created block that happens to contain
            // the word "momentum".
            const trimmedLabel = label.trim();
            const isLegacyMomentumLabel =
              trimmedLabel === 'app open' ||
              trimmedLabel === 'momentum' ||
              trimmedLabel === 'momentum generator';
            const isPointMarker = block.isPointMarker || isLegacyMomentumLabel;
          ...
          // Strip any legacy Wake Up POINTS (no endTime) from persisted data.
          // As of Task #86, Wake Up is no longer auto-generated at render time;
          // the Sleep block's end time is the single source of truth for when
          // the day starts. Wake Up blocks with endTime are user-created and
          // should be preserved.
          timeline = timeline.filter(b => {
